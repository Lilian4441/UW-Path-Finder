...
public class DijkstraGraph<NodeType, EdgeType extends Number>
  ...
  protected SearchNode computeShortestPath(NodeType start, NodeType end) {
    ...
    PlaceholderMap<NodeType,Double> helpMap = new PlaceholderMap<>();
    ...
    while (!pq.isEmpty()){
      ...
      if (!helpMap.containsKey((NodeType) currNode.node)){
        helpMap.put((NodeType) currNode.node, currNode.cost);
        for(Edge edge : currNode.node.edgesLeaving){
          pq.add(new SearchNode(edge.successor,
                  edge.data.doubleValue()+currNode.cost,currNode));
        }
      }
    }
    throw new NoSuchElementException();
  ...
  public List<NodeType> shortestPathData(NodeType start, NodeType end) {
    ...
    while (currNode != null){
      pathList.add(0,currNode.node.data);
